...
class Plotter:
    ...
    def plot_data(
        ...
        **kwargs: Any,
        ...
        Args:
            ...
            order (list of str): The order of the algo to plot.
        ...
        for o, p in zip(order, palette):
            print(f'algo: {o}, color: {p}')
        ax = sns.lineplot(
            data=data_to_plot,
            errorbar='sd',
            x=xaxis,
        ...
        if cost_limit:
            sub_figures[1].axhline(y=cost_limit, ls='--', c='black', linewidth=2)
        if save_name is None:
            save_name = all_logdirs[0].split('/')[-1]
        fig.savefig(
            f'./{save_name}-Reward.png',
            bbox_inches='tight',
            pad_inches=0.0,
        )
        ...
        if save_name is None:
            save_name = all_logdirs[0].split('/')[-1]
        fig.savefig(
            f'./{save_name}-Cost.png',
            bbox_inches='tight',
            pad_inches=0.0,
        )


    def get_datasets(self, logdir: str, condition: str | None = None):
        ...
        for root, _, files in os.walk(logdir):
            if 'progress.csv' in files:
                ...
                draw_orders.append(condition1)
        return datasets, draw_orders
    ...
    def make_plots(
        ...
        data, order = self.get_all_datasets(all_logdirs, legend, select, exclude)
